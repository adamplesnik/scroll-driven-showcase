...
const list: listType[] = [
  ...
  },
  {
    ...
    icon: Car,
    code: '<li class="animate-to-translate-x-0 timeline-view range-on-entry/60px-160px translate-x-[30vw]">',
    codeLink: 'https://github.com/adamplesnik/scroll-driven-playground/tree/main/src/pages/forest',
  },
]
    ...
    <div className="mb-24 mt-24">
      <div className="relative">
        <ul className="mb-28 flex flex-col gap-4">
          {list.map((item, i) => (
            <li
              className="flex translate-x-[30vw] animate-stroke-opacity gap-4 rounded-lg bg-slate-200/30 px-4 py-2 opacity-0 timeline-view range-on-entry/60px-160px dark:bg-zinc-800/60"
              key={i}
            >
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-2">
                ...
                {item.code && (
                  <CodeBlock language="html" linkHref={item.codeLink} Icon={Github}>
                    {item.code}
                  </CodeBlock>
                )}
              </div>
        ...
        <TimelineMarker className="bottom-[176px]" textStart="0px" textEnd="`on-entry`" />
        <TimelineMarker className="bottom-[116px]" textStart="60px" textEnd="0%" />
        <TimelineMarker className="bottom-[16px]" textStart="160px" textEnd="100%" />
      </div>
    </div>
  )
}
